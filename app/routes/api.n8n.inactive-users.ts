...
import { users, profiles, listings, campaignLog } from "~/lib/schema";
import { sql, eq } from "drizzle-orm";
import type { LoaderFunctionArgs } from "react-router";

/**
 * GET /api/n8n/inactive-users?hours=48&limit=50&exclude_campaign=skills-first-v1
 *
 * Secured endpoint for n8n to query users who signed up but haven't
 * created any listings/skills yet. Optionally excludes users who've
 * already received a specific campaign.
 *
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  //   - (Optional) Haven't received a specific campaign yet
    ...
    .where(
      sql`
        ...
        ${excludeCampaign ? sql`AND NOT EXISTS (
          ...
          WHERE ${campaignLog.userId} = ${users.id}
