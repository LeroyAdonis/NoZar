...
export function meta(_args: Route.MetaArgs) {
  return [
    { title: "Complaints & Dispute Resolution — NoZar" },
    {
      name: "description",
      content:
        "How to file a complaint or resolve a dispute on the NoZar barter platform. CPA and POPIA compliant.",
    },
  ];
}

export async function loader(_args: Route.LoaderArgs) {
  const filePath = resolve("docs/legal/complaints-process.md");
  const source = await readFile(filePath, "utf-8");
  const blocks = parseMarkdown(source);
  return { blocks };
}

export default function ComplaintsPage({ loaderData }: Route.ComponentProps) {
  return (
