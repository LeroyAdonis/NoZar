# Agent setup for `nozar`

This is a deepsec scanning workspace. Project `nozar` was just registered
(target: `..`). Setup is incomplete — `data/nozar/INFO.md`
...
2. **Fill in `data/nozar/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id nozar
pnpm deepsec process --project-id nozar
```
