...
Currently configured project: `nozar` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/nozar/SETUP.md` to fill in
   `data/nozar/INFO.md`.
...
`--project-id nozar` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/nozar/`.
