...
- **Global scope**: `C:/scratchpad/nozar/.kilo/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/scratchpad/nozar/.kilo/notes/`)
...
2. Glob `C:/scratchpad/nozar/.kilo/notes/*.md` (if directory exists) — global notes
...
Global (C:/scratchpad/nozar/.kilo/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/scratchpad/nozar/.kilo/notes/` — works in any directory
